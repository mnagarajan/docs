...
export default function PageLayout(props) {
  ...
  for (guideName in navConfig) {
    if (Object.prototype.hasOwnProperty.call(navConfig, guideName)) {
      guidesArray.push([guideName, navConfig[guideName]]);
    }
  }
  guidesArray = guidesArray.sort((a, b) => {
    const urlA = a[1].url, urlB = b[1].url;
  ...
  for (let index = 0; index < guidesArray.length; index++) {
    if (`${pathPrefix}${pathname}`.startsWith(guidesArray[index][1].url)) {
      guideName = guidesArray[index][0];
      break;
    }
  }
  ...
  return (
      <Layout>
        {/* BEGIN TUBEPRESS MODIFY */}
        <Helmet titleTemplate={`%s - ${guideName} - ${title}`}>
        {/* END TUBEPRESS MODIFY */}
          <link
              rel="stylesheet"
          ...
          >
            <HeaderInner>
              <ButtonWrapper ref={buttonRef}>
                ...
                >
                  {/* BEGIN TUBEPRESS MODIFY */}
                  <span className="title-sidebar">{guideName}</span>
                  {/* END TUBEPRESS MODIFY */}
                  <StyledIcon />
                </StyledButton>
              </ButtonWrapper>
